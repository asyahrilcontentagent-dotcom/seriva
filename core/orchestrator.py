...
class Orchestrator:
    ...
    def _generate_guarded_reply(
        ...
        *,
        ...
        # CEGAH RESPONSE LOOP - AMBIL 5 RESPONSE SEBELUMNYA
        last_responses = getattr(role_state, '_last_responses', [])
        
        temperature = self._get_llm_temperature(role_state)
        
        # Tambahkan anti-repetition instruction ke system message
        enhanced_messages = list(messages)
        if last_responses:
            last_three = "\n".join([f"- {resp[:100]}" for resp in last_responses[-3:]])
            anti_repeat_instruction = {
                "role": "system",
                "content": (
                    f"⚠️ PERINGATAN: Kamu TIDAK BOLEH mengulang kalimat berikut:\n{last_three}\n\n"
                    "Buat response yang SANGAT BERBEDA. Ganti struktur kalimat, tambahkan deskripsi sensasi, "
                    "gunakan variasi kata. JANGAN pakai inner thought (*deg*, *gumam*, dll). "
                    "LANJUTKAN ke aksi atau sensasi berikutnya, jangan stuck di kalimat yang sama."
                )
            }
        ...
        if last_responses:
            ...
            elif len(reply_text) > 40 and len(last_responses[-1]) > 40:
        ...
        if is_looping:
            ...
            force_fresh_messages.append({
                "role": "system",
                "content": "RESPONSE SEBELUMNYA TERULANG! KAMU HARUS BUAT RESPONSE YANG SANGAT BERBEDA!\n\n"
                           "JANGAN ulang kata-kata yang sama!\n"
                           "JANGAN pakai *bintang* atau inner thought!\n"
                           "JANGAN hanya bilang 'aku buka ini' - jelaskan SENSASINYA!\n"
                           "Ganti struktur kalimat total!\n"
                           "LANJUTKAN ke aksi berikutnya, jangan stuck!"
            })
            reply_text = self.llm.generate_text(
        ...
        # HAPUS INNER THOUGHT DAN BINTANG
        reply_text = self._remove_inner_thought(reply_text)
        
        # SIMPAN RESPONSE KE HISTORY (simpan 5 terakhir)
        if not hasattr(role_state, '_last_responses'):
            role_state._last_responses = []
        role_state._last_responses.append(reply_text.strip())
        if len(role_state._last_responses) > 5:
...
"""
    ...
    def _remove_inner_thought(self, text: str) -> str:
        """Hapus semua inner thought dan gestur dalam bintang yang mengganggu."""
        import re
        
        patterns = [
            r"\*deg\*", r"\*deg-degan\*", r"\*enak\*", r"\*sangek\*", r"\*becek\*",
            r"\*geli\*", r"\*achhh\*", r"\*uhuk\*", r"\*hufff\*", r"\*gemetar\*",
            r"\*lemas\*", r"\*gumam.*\*", r"\*pikir.*\*", r"\*dalam hati.*\*",
            r"\*kata hati.*\*", r"\*bisik.*\*", r"\*desis.*\*",
        ]
